...
int main(int argc, char* argv[])
{
  if (argc != 2)
    {
      fprintf(stderr, "Usage: %s script-without-dot-c\n", argv[0]);
      exit(1);
    }
  ...
  for (r = 0; r < 769; r++)
    {
      ...
      if (dinkdat.loc[r] != 0)
	{
	  struct small_map room;
	  if (load_map_to("MAP.DAT", dinkdat.loc[r], &room) < 0)
	    {
	      fprintf(stderr, "Cannot open DINK.DAT\n");
	      exit(1);
	    }
	  ...
	  for (s = 0; s < 101; s++)
	      ...
	      if (strcasestr(room.sprite[s].script, argv[1]) != NULL)
		{
		  printf("%s found in %d: sprite #%d - active=%d, vision=%d\n",
			 ...
			 s, room.sprite[s].active, room.sprite[s].vision);
		}
	    }
	}
    }
...
/**
 * Local Variables:
 * compile-command: "gcc -Wall -std=c99 -pedantic -g search_script.c -o search_script"
 * End:
 */
